...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function calculateStats(level, inventory) {
        ...
        let statsFromLevel = {
            ...
            healthRegen:
                baseStats.healthRegen + (currentAttributes.str - Attributes.str) * 0.1,
            attackSpeed:
                baseStats.attackSpeed + (currentAttributes.agl - Attributes.agl),
            ...
            manaRegen:
                baseStats.manaRegen + (currentAttributes.int - Attributes.int) * 0.05,
            magicResist:
                baseStats.magicResist + (currentAttributes.int - Attributes.int) * 0.1,
            evade: baseStats.evade + (level - 1) * 1.5,
            resist:
                baseStats.resist +
                (0.06 * baseStats.armor) / (1 + 0.06 * baseStats.armor),
        };
        ...
        if (healthRegenText)
            healthRegenText.textContent = `+${currentStats.healthRegen.toFixed(1)}`;
        const attackSpeedText = document.querySelector(".AttackSpeed");
        if (attackSpeedText)
            attackSpeedText.textContent = `${currentStats.attackSpeed.toFixed(0)}`;
        const damageText = document.querySelector(".Damage");
        if (damageText)
            damageText.textContent = `${currentStats.damage.toFixed(0)}`;
        ...
        if (manaRegenText)
            manaRegenText.textContent = `+${currentStats.manaRegen.toFixed(1)}`;
        const magicResistText = document.querySelector(".MagicResist");
        if (magicResistText)
            magicResistText.textContent = `${currentStats.magicResist.toFixed(1)}%`;
        ...
        if (resistText)
            resistText.textContent = `${currentStats.resist.toFixed(0)}%`;
        const moveSpeedText = document.querySelector(".MoveSpeed");
        if (moveSpeedText)
            moveSpeedText.textContent = currentStats.moveSpeed.toFixed(0);
        const manaRegenText2 = document.querySelector(".ManaRegen");
        if (manaRegenText2)
            manaRegenText2.textContent = currentStats.manaRegen.toFixed(2);
